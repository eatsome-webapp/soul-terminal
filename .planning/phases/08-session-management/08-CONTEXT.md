...
Tab bar bovenaan de terminal sheet voor sessie-navigatie. GestureDetector voor swipe tussen sessies. Process namen live gelezen uit `/proc/PID/cmdline`. SOUL chat kan sessies aanmaken, sluiten en wisselen via Pigeon. Vervangt de drawer-gebaseerde sessielijst als primaire sessie-navigatie.
...
- Settings button verplaatsen naar tab bar area (overflow menu of icoon in drag handle) — drawer-swipe conflicteert met GestureDetector horizontale swipe
- Drawer swipe-from-edge uitschakelen (`setDrawerLockMode(LOCK_MODE_LOCKED_CLOSED)`) om conflict met horizontale swipe te voorkomen

### Session switching mechanisme
- TabLayout programmatisch gesynchroniseerd met sessie-lijst (geen ViewPager2, geen TabLayoutMediator)
- GestureDetector op TerminalView vangt horizontale fling op en roept `switchToSession(true/false)` aan
- TerminalView is een enkele view die van sessie wisselt via `setCurrentSession()`
...
- **`TermuxActivity.java`**: Tab bar initialisatie, GestureDetector setup, programmatische tab synchronisatie
...
- GestureDetector swipe moet soepel aanvoelen — geen conflict met terminal scroll (verticaal) of drawer (horizontaal, wordt gelocked)
